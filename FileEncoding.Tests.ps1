...
Describe 'File Encoding' -Tags 'Files' {
    ...
    Context -Name 'Source File Testing' -Fixture {
        It -Name 'Should have file encoding UTF8 for file <BaseName>' -TestCases $sourceFileCases -Test {
            param
            (
                [String]
                $FullName
            )
            Get-FileEncoding -Path $FullName | Should -BeIn ('UTF8', 'ascii')
        }
    }
    Context -Name 'Test File Testing' -Fixture {
        It -Name 'Should have file encoding UTF8 for file <BaseName>' -TestCases $testFileCases -Test {
            param
            (
                [String]
                $FullName
